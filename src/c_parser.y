...
%type <bnode> argument_expression_list parameter_list initialiser_list struct_declaration_list struct_declarator_list enumerator_list split_func_list split_fpga_func_list
...
%type <node> block_item expression_statement selection_statement iteration_statement labeled_statement split_statement split_fpga_statement struct_declaration
...
%type <enode> constant_expression split_func_item split_fpga_func_item
...
direct_declarator:
		...
		| direct_declarator '(' ')' 						{$$ = new FunctionDeclaration($1);}	//used for function definitions with no args
		;
...
direct_abstract_declarator:
		...
		| '(' ')'													{/*TODO: do it later.*/}
		| '(' parameter_type_list ')'								{/*TODO: do it later.*/}
		| direct_abstract_declarator '(' ')'						{/*TODO: do it later.*/}
		| direct_abstract_declarator '(' parameter_type_list ')'	{/*TODO: do it later.*/}
		;
...
statement:
		...
		| jump_statement						{$$ = $1;}
		| split_statement						{$$ = $1;}
		| split_fpga_statement					{$$ = $1;}
		;


split_fpga_statement:
		T_SPLIT_FPGA '{' split_fpga_func_list '}'			{ $$ = $3; }
		;
...
split_fpga_func_item:
		T_IDENTIFIER assignment_operator T_IDENTIFIER '(' ')' ';' 				{$$ = new split_fpga_func_item(*($1),*($3),NULL);}
		| T_IDENTIFIER assignment_operator T_IDENTIFIER '(' argument_expression_list ')' ';' {$$ = new split_fpga_func_item(*($1),*($3),$5);}
		;
